...
let
    ...
    in
      pkgs.runCommand "lint" {} ''
        mkdir -p $out/bin
        ln -s ${lint-python}/bin/black $out/bin
        ln -s ${lint-python}/bin/flake8 $out/bin
        ln -s ${lint-python}/bin/isort $out/bin
      '';
...
in
{
  inherit privatestorage lint-env tests;
}
